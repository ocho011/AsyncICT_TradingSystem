...
from domain.entities.LiquidityPool import LiquidityPool, LiquidityType
...
class AsyncLiquidityDetector:
    ...
    async def start_detection(self):
        ...
        topic = f"candle:{self.symbol}:1m" # Example: using 1m timeframe for liquidity events
    ...
    async def _detect_liquidity_sweep(self, candle: CandleEvent):
        ...
        for high in list(self.swing_highs)[:-1]: # Exclude the most recent high
            if candle.low < high < candle.high:
                ...
                await self.event_bus.publish(MarketEvents.LIQUIDITY_SWEEP_DETECTED, sweep_event)
